...
class CommonSetup(aetest.CommonSetup):
    ...
    def establish_connections(self):
        ...
        Test.start_step(f"Ssh into {target_router}")(Test.connect_to_uut)()
    ...
    def backup_configs(self):
        ...
        for device_name in devices_to_back_up:
            Test.start_step(f"Back up current configurations for {device_name}")(
                Test.backup_running_config
            )(target_router=device_name)
    ...
    def initialize_ixia_traffic_items(self):
        if "tgn" in Test.test_data:
            Test.start_step("Connect to traffic generator")(Test.connect_to_tgn)()

            Test.start_step("Identify IXIA traffic items")(Test.identify_ixia_traffic)()

            Test.start_step("Disable all ixia traffic")(Test.disable_all_ixia_traffic)()

    @aetest.skipUnless(Test.keep_running(), "Automation has failed already")
    @aetest.subsection
    def set_test_parameters(self):
        Test.start_step("Set test parameters")(Test.set_test_parameters)()

        testcases = Test.start_step("Get testcases")(Test.get_testcases)(__name__)

        Test.start_step("Skip inactive testcases")(Test.skip_inactive_tests)(testcases)

    @aetest.skipUnless(Test.keep_running(), "Automation has failed already")
    @aetest.subsection
    def take_topology_snapshot(self):
        Test.start_step("Get topology")(Test.save_original_topology)(
            cli_criteria="inc <leaf names' common prefix>"
        )

    @aetest.skipUnless(Test.keep_running(), "Automation has failed already")
    @aetest.subsection
    def ensure_bgp_is_stable(self):
        Test.start_step("Clear BGP sessions")(Test.clear_bgp_sessions_hard)()

        Test.troubleshootable_step(
            "Wait for BGP to converge", troubleshoot_categories="bgp_convergence"
        ...
        Test.start_step("Wait for BGP peers to change")(Test.wait)(5 * 60)
...
class xr_ecmp_test1(aetest.Testcase):
    ...
    def testcase_setup(self, steps, section):
        Test.set_params(
            section=section, testcase=f"{self.__class__.__name__}", steps=steps
        )
        Test.start_step(
            f"Display test plan for {self.__class__.__name__}", continue_=True
        )(Test.testcase_overview)(self.__class__.__name__)

        Test.start_step(f"Clear log and context")(Test.run_cmds)(
            ["clear logging", "clear context"]
        )

        Test.start_step(f"Shut down BGP neighbors on <router1's name>")(
            Test.apply_testcase_configs
        )(1)

        Test.start_step("Start ixia traffic")(Test.start_ixia_traffic)(
            1, max_apply_attempts=7
        )
    ...
    def _verify_ecmp(self, num_paths, threshold: int = 0.9, stdev: int = 2):
        ...
        changes_made = Test.start_step(f"Keep {num_paths} interfaces")(
            Test.keep_x_interfaces_unshut
        )(
            target_count=num_paths,
            interested_devices=interested_devices,
            sort="target",
            lldp_cli_criteria="inc <leaf names' common prefix>",
        )

        if changes_made:
            Test.start_step("Wait for all interfaces to stabilize")(Test.wait)(5 * 60)
        ...
        Test.troubleshootable_step(
            f"Verify that {num_paths} paths exists in the BGP table",
            continue_=continue_,
            troubleshoot_categories="bgp_table",
        )(Test.verify_line_count)(
            "show bgp ipv6 unicast {multipath_ip}",
            "inc multipath",
            "==",
            num_paths,
            f"{num_paths} paths in the BGP table",
        )

        traffic_data_by_rtsws = Test.start_step(f"Get outbound packets for each rtsw")(
            Test.get_traffic_stats_for_each_device
        )(interested_devices=interested_devices, stat_name="OutMbps")
        ...
        Test.start_step(
            f"Ensure the outbound packet rates for all interfaces fall within 3 standard deviations of the average rate"
        )(Test.verify_similar_rates)(
            data=all_traffic_data, threshold=threshold, stdev=stdev
        )
    ...
    def verify_no_traffic_loss(self):
        Test.start_step("Stop IXIA traffic")(Test.stop_ixia_traffic)(1)

        Test.wait(10)

        Test.start_step("Verify no traffic loss")(Test.check_traffic_loss)(
            traffic_num=1
        )
    ...
    def testcase_cleanup(self):
        ...
        Test.start_step("Check traceback and dumps")(Test.check_traceback_dumps)()
...
class CommonCleanup(aetest.CommonCleanup):
    ...
    @aetest.subsection
    def upload_log(self):
        Test.start_step("Upload log", continue_=True)(Test.upload_log)()
